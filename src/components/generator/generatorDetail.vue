<template>
  <div class="generator-detail-div">
    <grid :cols="4" :show-lr-borders="false" class="status-grid" :show-vertical-dividers="false">
      ...
      <grid-item class="status-grid-item">
        <div>类型: {{generatorData.stationType}}</div>
      </grid-item>
      <grid-item class="status-grid-item">
        <div>市电: {{generatorData.Acity_electricity}}</div>
      </grid-item>
      <grid-item class="status-grid-item">
        <div>GPRS: <i class="iconfont icon-signal " v-if="generatorData.st_state!='离线'"></i><i
    ...
    <!--<div>温度: {{generatorData.external_temperature|keepHowManyNum(1)}}°C</div>-->
    ...
    <grid :cols="3" :show-lr-borders="false" :show-vertical-dividers="false">
      ...
      <grid-item>
        ...
        <div class="text-align-center detail-content">{{generatorData.st_current|translateCurrent|keepHowManyNum(1)}}A</div>
